...
export const login = (email, password) => async (dispatch) => {
    try {
          ...
          const { data } = await axios.post('https://localhost:4000/api/users/login', { email, password }, config)
...
export const register = (name, email, password, isAdmin) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post('https://localhost:4000/api/users/', { name, email, password, isAdmin },
            config
        )
